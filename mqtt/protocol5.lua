...
local function parse_packet_pubrel(ptype, flags, input)
	...
	-- If Remaining Length > 2, we have at least a Reason Code
	if input.available > 0 then
		-- DOC: 3.6.2.1 PUBREL Reason Code (Byte 3)
		local rc
		rc, err = parse_uint8(read_data)
		if not rc then
			return false, packet_type[ptype]..": failed to parse rc: "..err
		...
		-- DOC: 3.6.2.2.1 If Remaining Length > 3, we have a Property Length field (Byte 4)
		if input.available > 0 then
...
local function parse_packet_pubcomp(ptype, flags, input)
	...
	-- If Remaining Length > 2, we have at least a Reason Code
	if input.available > 0 then
		-- DOC: 3.7.2.1 PUBCOMP Reason Code (Byte 3)
		local rc
		rc, err = parse_uint8(read_data)
		if not rc then
			return false, packet_type[ptype]..": failed to parse rc: "..err
		...
		-- DOC: 3.7.2.2.1 If Remaining Length > 3, we have a Property Length field (Byte 4)
		if input.available > 0 then
